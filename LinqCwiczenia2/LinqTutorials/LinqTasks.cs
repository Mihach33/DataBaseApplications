...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<object> result = from e in Emps
                                         group e by e.Job into z
                                         select new
                                         {
                                             Praca = z.Key,
                                             Count = z.Count()
                                         };
            //Emps.GroupBy(e => e.Job).Select(j => new { Praca = j.Job, LiczbaPracownikow = Emps.Count() });
        ...
        {
            Emp result = Emps.Where(e => e.Job == "Frontend programmer").OrderByDescending(e => e.Job).First();
            return result;
        }
        ...
        public static IEnumerable<object> Task10()
        {
            ...
            n.Add(new { a = "Brak wartości", b = (string?)null, c = (string?)null });

            IEnumerable<object> result = Emps.Select(e => new { e.Ename, e.Job, e.HireDate }).Union(n);
            return result;
        }
        ...
        {
            int result = 0;
            result = arr.GroupBy(n => n).Where(n => n.Count() % 2 == 1).Select(n => n.Key).Single();
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result = Depts
                .Where(d => !Emps.Where(e => e.Deptno != null)
                .Select(e => e.Deptno).ToList().Contains(d.Deptno)).Select(d => d)
                .Union(
                    Depts
                    .Join(Emps,
                        d => d.Deptno,
